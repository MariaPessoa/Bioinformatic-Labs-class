...
def swissprot(filename):
    ...
    as an argument and prints the ID field, with the revision status and 
    seqeunce length, as well as the gene name field, source organism,
    subcellular location and Prosite domains.
    ...
    for i in range(len(file_list)):
        if "ID" in file_list[i][:5]: 
            print("ID, Status & length:",file_list[i][5:].replace("    ",""),end="")
        if "GN" in file_list[i][:5]: print("Gene Name:",file_list[i][5:],end="")
        if "OS" in file_list[i][:5]: print("Source:",file_list[i][5:],end="")
        if "SUBCELLULAR LOCATION" in file_list[i][:30]: 
            print("Subcellular Location:",file_list[i][30:],end="")
        if "DR" in file_list[i][:5] and "PROSITE" in file_list[i][:15]:
            print("Prosite domains:",file_list[i][5:],end="")
    return None
...
def menu():
    while True:
        ...
        if op == "1": 
            ...
            if user == 1: print("Acession numbers in file:",
                                blastinfo("Blast_irf4_on.xml"))
            elif user == 2: print("Acession numbers in file:",
                                  blastinfo("Blast_mc1r_on.xml"))
            elif user == 3: print("Acession numbers in file:",
                                  blastinfo("Blast_mitf_on.xml"))
            else: print("Invalid option")
        elif op == "5": 
            user = int(input("IRF4(1), MC1R(2) or MITF(3)? "))
